...
ELEVEURS_URL = "http://bovin.atwebpages.com/api/eleveurs.php"
ANIMALS_URL  = "http://bovin.atwebpages.com/api/animaux_eleveur.php"  # ?eleveur_id=X
...
TRANSLATIONS = {
    "FR": {
        ...
        "conf_suffix": "%",
        # ── Éleveurs ──
        "eleveurs_title": "### 👨‍🌾 Éleveurs & leurs animaux",
        "eleveur_expand": "👤 Voir les animaux de",
        "eleveur_no_animal": "Aucun animal trouvé pour cet éleveur.",
        "eleveur_load_err": "Impossible de charger les éleveurs.",
        "col_e_nom": "👤 Nom",
        "col_e_email": "📧 Email",
        "col_e_tel": "📞 Téléphone",
        "col_a_nom": "🐄 Animal",
        "col_a_race": "🧬 Race",
        "col_a_sexe": "⚥ Sexe",
        "col_a_nais": "📅 Naissance",
        "col_a_temp": "🌡️ Temp (°C)",
        "col_a_activ": "🏃 Activité (%)",
        "col_a_statut": "📊 Statut",
        "no_eleveur": "Aucun éleveur enregistré.",
        "animals_of": "animaux",
    },
    "EN": {
        ...
        "conf_suffix": "%",
        "eleveurs_title": "### 👨‍🌾 Breeders & their animals",
        "eleveur_expand": "👤 Show animals of",
        "eleveur_no_animal": "No animals found for this breeder.",
        "eleveur_load_err": "Could not load breeders.",
        "col_e_nom": "👤 Name",
        "col_e_email": "📧 Email",
        "col_e_tel": "📞 Phone",
        "col_a_nom": "🐄 Animal",
        "col_a_race": "🧬 Breed",
        "col_a_sexe": "⚥ Sex",
        "col_a_nais": "📅 Birth date",
        "col_a_temp": "🌡️ Temp (°C)",
        "col_a_activ": "🏃 Activity (%)",
        "col_a_statut": "📊 Status",
        "no_eleveur": "No breeders registered.",
        "animals_of": "animals",
    },
    "AR": {
        ...
        "conf_suffix": "%",
        "eleveurs_title": "### 👨‍🌾 المربّون وحيواناتهم",
        "eleveur_expand": "👤 عرض حيوانات",
        "eleveur_no_animal": "لا توجد حيوانات لهذا المربّي.",
        "eleveur_load_err": "تعذّر تحميل قائمة المربّين.",
        "col_e_nom": "👤 الاسم",
        "col_e_email": "📧 البريد",
        "col_e_tel": "📞 الهاتف",
        "col_a_nom": "🐄 الحيوان",
        ...
        "col_a_sexe": "⚥ الجنس",
        "col_a_nais": "📅 الميلاد",
        "col_a_temp": "🌡️ الحرارة (°C)",
        "col_a_activ": "🏃 النشاط (%)",
        "col_a_statut": "📊 الحالة",
        "no_eleveur": "لا يوجد مربّون مسجّلون.",
        "animals_of": "حيوانات",
    }
}
...
.btn-actualiser .stButton > button { background-color: #1565c0 !important; }
.btn-actualiser .stButton > button:hover { background-color: #1976d2 !important; }

.btn-systeme .stButton > button { background-color: #546e7a !important; }
.btn-systeme .stButton > button:hover { background-color: #607d8b !important; }
...
.alert-critique { background-color: #ffebee; border-left: 4px solid #e53935; color: #b71c1c; }
.alert-alerte   { background-color: #fff8e1; border-left: 4px solid #fb8c00; color: #e65100; }
.alert-normal   { background-color: #e8f5e9; border-left: 4px solid #43a047; color: #1b5e20; }
...
.eleveur-name { font-weight: 700; font-size: 15px; color: #1b5e20; }
.eleveur-meta { font-size: 12px; color: #555; }
.badge-count {
...
/* statut pills */
.pill-normal   { background:#e8f5e9; color:#1b5e20; border:1px solid #a5d6a7; border-radius:12px; padding:2px 9px; font-size:11px; font-weight:600; }
.pill-alerte   { background:#fff8e1; color:#e65100; border:1px solid #ffcc80; border-radius:12px; padding:2px 9px; font-size:11px; font-weight:600; }
.pill-critique { background:#ffebee; color:#b71c1c; border:1px solid #ef9a9a; border-radius:12px; padding:2px 9px; font-size:11px; font-weight:600; }
...
def load_eleveurs():
    """
    Retourne la liste des éleveurs depuis l'API.
    Format attendu :
      [{"id":1,"first_name":"Ahmed","last_name":"Ben Ali","email":"...","telephone":"..."}]
    Si l'API n'existe pas encore, on tente de reconstruire depuis ia_data.
    """
    try:
        r = requests.get(ELEVEURS_URL, timeout=10)
        data = r.json()
        ...
        # Fallback : extraire les éleveurs depuis ia_data si possible
        try:
            r2 = requests.get(API_URL, timeout=10)
            d2 = r2.json()
            resultats = d2.get("resultats_ia", [])
            seen = {}
            for item in resultats:
                eid = item.get("eleveur_id") or item.get("user_id")
                enom = item.get("eleveur_nom") or item.get("eleveur_name") or "?"
                if eid and eid not in seen:
                    parts = str(enom).split(" ", 1)
                    seen[eid] = {
                        "id": eid,
                        "first_name": parts[0] if parts else enom,
                        "last_name": parts[1] if len(parts) > 1 else "",
                        "email": item.get("eleveur_email", ""),
                        "telephone": item.get("eleveur_tel", ""),
                    }
...
@st.cache_data(ttl=30)
def load_animaux_eleveur(eleveur_id: int):
    """
    Retourne les animaux d'un éleveur donné.
    Format attendu :
      [{"nom":"Bella","race":"Montbéliarde","sexe":"F","date_naissance":"2021-03-12",
        "temperature":38.5,"niveau_activite":72,"statut_sante":"normal"}]
    """
    try:
        r = requests.get(ANIMALS_URL, params={"eleveur_id": eleveur_id}, timeout=10)
        data = r.json()
        if isinstance(data, list):
            return data
        if isinstance(data, dict) and "data" in data:
        ...
        # Fallback : filtrer depuis ia_data
        try:
            r2 = requests.get(API_URL, timeout=10)
            d2 = r2.json()
            resultats = d2.get("resultats_ia", [])
            return [
                a for a in resultats
                if str(a.get("eleveur_id") or a.get("user_id", "")) == str(eleveur_id)
            ]
        except Exception:
            return []
...
# ─── GRAPHIQUES ──────────────────────────────────────────────────────────────
color_map = {"normal": "#4caf50", "alerte": "#ff9800", "critique": "#f44336"}
...
if results:
    ...
    with col_g1:
        ...
        fig_pie = px.pie(
            ...
            title=T["pie_title"], color="Statut",
...
# ═══════════════════════════════════════════════════════════════════════════════
# ─── SECTION ÉLEVEURS & LEURS ANIMAUX ─────────────────────────────────────────
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown(T["eleveurs_title"])

eleveurs = load_eleveurs()

if not eleveurs:
    st.info(T["no_eleveur"])
else:
    for elev in eleveurs:
        eid        = elev.get("id") or elev.get("user_id") or elev.get("eleveur_id")
        prenom     = elev.get("first_name") or elev.get("prenom") or ""
        nom_fam    = elev.get("last_name")  or elev.get("nom")    or ""
        full_name  = f"{prenom} {nom_fam}".strip() or f"Éleveur #{eid}"
        email      = elev.get("email", "—")
        telephone  = elev.get("telephone") or elev.get("tel") or "—"
        initiale   = prenom[0].upper() if prenom else (nom_fam[0].upper() if nom_fam else "?")

        # ── charger les animaux de cet éleveur ──
        animaux = load_animaux_eleveur(int(eid)) if eid else []
        nb_animaux = len(animaux)

        with st.expander(
            f"👤 {full_name}   —   {nb_animaux} {T['animals_of']}",
            expanded=False
        ):
            # ── Fiche éleveur ──
            st.markdown(
                f"""<div class="eleveur-card">
                    <div class="eleveur-header">
                        <div class="eleveur-avatar">{initiale}</div>
                        <div>
                            <div class="eleveur-name">{full_name}</div>
                            <div class="eleveur-meta">📧 {email} &nbsp;&nbsp; 📞 {telephone}</div>
                        </div>
                        <span class="badge-count">{nb_animaux} {T['animals_of']}</span>
                    </div>
                </div>""",
                unsafe_allow_html=True
            )
            ...
            if not animaux:
                ...
                for a in animaux:
                    ...
                    pill_cls = {
                        ...
                        "alerte":   "pill-alerte",
                    }.get(statut_raw, "pill-normal")
                    pill_html = f'<span class="{pill_cls}">{statut_raw}</span>'

                    temp   = a.get("temperature") or a.get("temp") or "—"
                    activ  = a.get("niveau_activite") or a.get("activite") or "—"
                    race   = a.get("race") or a.get("breed") or "—"
                    sexe   = a.get("sexe") or a.get("sex") or "—"
                    naiss  = a.get("date_naissance") or a.get("naissance") or a.get("birthdate") or "—"
                    nom_a  = a.get("nom") or a.get("nom_animal") or a.get("name") or f"Animal #{a.get('id','?')}"
                    ...
                    rows.append({
                        ...
                        T["col_a_temp"]   : temp,
                        T["col_a_activ"]  : activ,
                        T["col_a_statut"] : statut_raw,
                        "_pill"           : pill_html,
                    })

                df_anim = pd.DataFrame(rows)

                # Statistiques rapides de l'éleveur
                n_crit = sum(1 for r in rows if r[T["col_a_statut"]] == "critique")
                n_aler = sum(1 for r in rows if r[T["col_a_statut"]] == "alerte")
                n_norm = nb_animaux - n_crit - n_aler

                mc1, mc2, mc3 = st.columns(3)
                mc1.metric("✅ Normal",   n_norm)
                mc2.metric("⚠️ Alerte",   n_aler)
                mc3.metric("🚨 Critique", n_crit)

                # Tableau principal (sans la colonne _pill cachée)
                cols_display = [
                    T["col_a_nom"], T["col_a_race"], T["col_a_sexe"],
                    T["col_a_nais"], T["col_a_temp"], T["col_a_activ"], T["col_a_statut"]
                ]

                # Coloration des lignes selon statut
                def color_row(row):
                    s = row[T["col_a_statut"]]
                    if s == "critique":
                ...
                st.dataframe(
                    ...
                    hide_index=True,
                    height=min(80 + nb_animaux * 38, 500),
                )

                # Mini graphique statuts si > 2 animaux
                if nb_animaux > 2:
                    stat_counts = df_anim[T["col_a_statut"]].value_counts().reset_index()
                    ...
                    fig_mini.update_layout(
                        ...
                        plot_bgcolor="rgba(241,248,233,0.5)",
